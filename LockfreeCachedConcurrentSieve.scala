...
object LockfreeCachedConcurrentSieve{
  ...
  def main(args: Array[String]) = {
    ...
    def comp(me: Int) {
      // Stores the last index that this thread inserted into the primes array
      var lastIndex = 0;
      ...
      while(nextSlot.get<N) {
        ...
        if(p == 0 || p*p>candidate){ // candidate is prime
          ...
          var ii = lastIndex
          while (ii <= nextSlot.get && primeToInsert != 0) {
